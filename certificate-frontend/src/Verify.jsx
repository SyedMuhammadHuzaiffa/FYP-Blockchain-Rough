...
// Uses new contract directly — no registry needed

import React, { useState, useEffect, useRef } from "react";
import { ethers } from "ethers";
import { filebaseGatewayUrl } from "./ipfsClient";

// ─── New contract — direct address ────────────────────────────────────────────
...
// ─── Verify using new contract — no wallet needed ─────────────────────────────
async function verifyOnChain(cid) {
  const provider = await getPublicProvider();
  const contract = new ethers.Contract(CONTRACT_ADDRESS, VERIFY_ABI, provider);
  const cleanCid = decodeURIComponent(cid.trim());
  const result   = await contract.verify(cleanCid);
  return {
    ...
    <div style={{ borderRadius:14, border:"2px solid #22c55e", overflow:"hidden", background:"#fff", boxShadow:"0 4px 24px rgba(0,0,0,0.08)" }}>
      <div style={{ padding:"14px 20px", background:"#14532d", display:"flex", alignItems:"center", gap:10 }}>
        ...
        <div>
          ...
          <div style={{ fontSize:11, color:"#86efac", marginTop:2 }}>Verified via Polygon Amoy — no wallet required</div>
        </div>
      </div>
        ...
        <div style={{ flex:1, minWidth:200 }}>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12, marginBottom:16 }}>
            ...
            <div>
              ...
              <div style={{ fontSize:10, color:"#334155", fontFamily:"monospace", wordBreak:"break-all", lineHeight:1.4 }}>{cid}</div>
            </div>
          </div>
    ...
    <div style={{ borderRadius:14, border:"2px solid #ef4444", overflow:"hidden", background:"#fff", boxShadow:"0 4px 24px rgba(0,0,0,0.08)" }}>
      <div style={{ padding:"14px 20px", background:"#7f1d1d", display:"flex", alignItems:"center", gap:10 }}>
        ...
        <div>
          ...
          <div style={{ fontSize:11, color:"#fca5a5", marginTop:2 }}>Does not exist on Polygon Amoy</div>
        </div>
      </div>
      <div style={{ padding:20 }}>
        <p style={{ fontSize:14, color:"#475569", lineHeight:1.7, margin:"0 0 12px" }}>
          ...
          It may be <strong>fake</strong>, <strong>invalid</strong>, or issued before the new contract.
        </p>
        {cid && (
        ...
        <div style={{ marginTop:12, fontSize:12, color:"#92400e", background:"#fefce8", padding:"10px 14px", borderRadius:8, border:"1px solid #fde68a" }}>
          💡 Make sure you copied the <strong>full</strong> Certificate ID from the email link.
        </div>
      </div>
    </div>
  );
    ...
    <div style={{ paddingBottom:60 }}>
      ...
      <div style={{ textAlign:"center", padding:"36px 16px 24px" }}>
        ...
        <p style={{ maxWidth:500, margin:"0 auto", color:"#64748b", fontSize:14, lineHeight:1.7 }}>
          Enter the Certificate ID or scan the QR code.<br />
          Verified on <strong>Polygon Amoy blockchain</strong>.<br />
      ...
      <div style={{ maxWidth:720, margin:"0 auto", padding:"0 16px" }}>
        <form onSubmit={(e) => { e.preventDefault(); runVerify(inputCid); }}>
          <div style={{ display:"flex", gap:8, flexWrap:"wrap" }}>
            ...
            <button type="submit" disabled={verifying} style={{ padding:"13px 24px", background:"#2563eb", border:"none", borderRadius:10, color:"#fff", fontWeight:700, fontSize:14, cursor: verifying?"not-allowed":"pointer", opacity: verifying?0.7:1, whiteSpace:"nowrap" }}>
              {verifying ? "⏳ Verifying..." : "Verify"}
            </button>
            <button type="button" onClick={() => setShowScanner(true)} style={{ padding:"13px 18px", background:"#f0f9ff", border:"1px solid #7dd3fc", borderRadius:10, color:"#0369a1", fontWeight:600, fontSize:14, cursor:"pointer", whiteSpace:"nowrap" }}>
              📷 Scan QR
            </button>
          </div>
        </form>
        ...
        {!result && !notFound && !verifying && !error && (
          <div style={{ marginTop:32, display:"flex", gap:10, flexWrap:"wrap", justifyContent:"center" }}>
            {[
              ...
              { n:"2", text:"Paste above or scan QR code" },
